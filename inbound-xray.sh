...
# --- Определение Docker-режима ------------------------------------------------
# Скрипт работает НА ХОСТЕ. Docker-режим определяется по наличию docker-compose.yml
# рядом со скриптом (в корне проекта).
...
# Адрес xray-инбаундов в proxy_pass / stream map:
#   bare-metal: 127.0.0.1 (localhost)
#   Docker:     имя контейнера 3x-ui (bridge-сеть)
_upstream() {
...
nginx_reality_target_server() {
    ...
    # bare-metal: listen 127.0.0.1 (только localhost); Docker: listen 0.0.0.0 (доступен из контейнера 3x-ui).
    local listen_addr="127.0.0.1"
    is_docker_mode && listen_addr="0.0.0.0"
    {
        printf '%s\n' "# inbound-xray.sh: REALITY target-заглушка (${listen_addr}:${port}, ${root_dom:-<домен>})"
        printf '%s\n' "server {"
...
require_panel() {
    ...
    if is_docker_mode; then
        docker compose -f "$COMPOSE_FILE" ps 3x-ui 2>/dev/null | grep -q "Up" \
            || warn "Контейнер 3x-ui не активен. Настройка продолжится, но для применения нужен запуск контейнера."
    else
...
load_panel_env() {
    ...
    if is_docker_mode && [[ -x "${DOCKER_DIR}/3x-ui/x-ui" ]]; then
        xui_bin="${DOCKER_DIR}/3x-ui/x-ui"
    fi
    if [[ -x "$xui_bin" ]]; then
    ...
    info "Панель: ${PANEL_PROTO}://$(_upstream):${PANEL_PORT}${PANEL_PATH:-/} (webBasePath: ${PANEL_PATH:-/})"
}
...
firewall_port_open() {
    ...
    if is_docker_mode; then
        # В Docker порты управляются через docker-compose.yml (ports:).
        # ufw/firewalld не нужны для инбаундов — всё через 443.
        return 0
    fi
...
restart_xui() {
    if is_docker_mode; then
        info "Перезапуск панели x-ui (Docker: docker compose restart 3x-ui)..."
        if ! docker compose -f "$COMPOSE_FILE" restart 3x-ui 2>&1; then
            fail "docker compose restart 3x-ui завершился с ошибкой."
            return 1
        fi
        sleep 2
        if docker compose -f "$COMPOSE_FILE" ps 3x-ui 2>/dev/null | grep -q "Up"; then
            ok "Контейнер 3x-ui перезапущен и активен."
        else
            fail "Контейнер 3x-ui не запустился — проверьте логи (docker compose logs 3x-ui)."
            return 1
        fi
    elif command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files x-ui.service >/dev/null 2>&1; then
...
nginx_stream_master_rebuild() {
    ...
    local rows=""
    rows="$(sqlite3 "$XUI_DB" "SELECT id, port, listen, replace(stream_settings, char(10), char(32)) FROM inbounds WHERE protocol IN ('vless','vmess','trojan') AND port != ${STREAM_MASTER_PORT:-443} AND (listen = '127.0.0.1' OR listen = '' OR listen IS NULL);" 2>/dev/null || true)"
    while IFS= read -r row; do
        ...
        if [[ -n "$sni" ]]; then
            if [[ -z "$(stream_sni_in_map "$map_lines" "$sni")" ]]; then
                map_lines="${map_lines}    ${sni}  $(_upstream):${port};
"
            else
...
"
    ...
    if [[ -n "$PANEL_HOST" && -n "$PANEL_SSL_PORT" ]]; then
        ...
        if [[ -n "$busy_port" ]]; then
            ...
            panel_line="    ${PANEL_HOST}  $(_upstream):${PANEL_SSL_PORT};
"
        fi
    fi
    ...
    if [[ -n "$root_dom" && "$root_dom" != "$PANEL_HOST" && -n "$PANEL_SSL_PORT" \
        ...
        if [[ -n "$busy_port" ]]; then
            ...
            root_line="    ${root_dom}  $(_upstream):${PANEL_SSL_PORT};
"
        fi
    fi
    cat > "$tmp" <<EOF
...
${map_lines}${panel_line}${root_line}    default  $(_upstream):${real_def};
}
...
stream_master_apply_proxy_protocol() {
    ...
    rows="$(sqlite3 "$XUI_DB" "SELECT id, replace(stream_settings, char(10), char(32)) FROM inbounds WHERE tag LIKE 'inbound-%' AND protocol IN ('vless','vmess','trojan') AND (listen = '127.0.0.1' OR listen = '' OR listen IS NULL) AND port != ${STREAM_MASTER_PORT:-443};" 2>/dev/null || true)"
    while IFS= read -r row; do
        [[ -z "$row" ]] && continue
        id="${row%%|*}"; ss="${row#*|}"
        printf '%s' "$ss" | grep -qE '"network"[[:space:]]*:[[:space:]]*"(tcp|xhttp)"' || continue
...
WHERE i.protocol = 'vless'
  AND (i.listen = '127.0.0.1' OR i.listen = '' OR i.listen IS NULL)
...
nginx_stream_master_setup() {
    ...
    if db_has_column hosts port; then
        sqlite3 "$XUI_DB" "UPDATE hosts SET port = ${STREAM_MASTER_PORT:-443} WHERE inbound_id IN (SELECT id FROM inbounds WHERE tag LIKE 'inbound-%' AND protocol IN ('vless','vmess','trojan') AND (listen = '127.0.0.1' OR listen = '' OR listen IS NULL));" 2>/dev/null || true
    fi
...
nginx_stream_rebuild_legacy() {
    ...
    local rows=""
    rows="$(sqlite3 "$XUI_DB" "SELECT port, listen, replace(stream_settings, char(10), char(32)) FROM inbounds WHERE protocol IN ('vless','vmess','trojan') AND (listen = '127.0.0.1' OR listen = '' OR listen IS NULL) AND port != ${STREAM_MASTER_PORT:-443};" 2>/dev/null || true)"
    while IFS= read -r row; do
        [[ -z "$row" ]] && continue
        port="${row%%|*}"; row="${row#*|}"
        listen="${row%%|*}"; ss="${row#*|}"
...
delete_channel() {
    ...
    for r in "${del_records[@]}"; do
        ...
        # Через 443 (listen=127.0.0.1 или '' в Docker) выходили инбаунды за nginx —
        # для них правило firewall не создавалось, удалять нечего.
        if [[ "$del_listen" != "127.0.0.1" && -n "$del_listen" ]]; then
            del_remove=1
        fi
...
print_summary() {
    ...
    if [[ "$LISTEN" == "127.0.0.1" || ( "$LISTEN" == "" && "$STREAM_443_MASTER" == "1" ) ]]; then
...
write_panel_conf() {
    ...
    if [[ "$SUB_ENABLE" == "true" && -n "$SUB_PORT" ]]; then
        ...
        [[ "$sub_loc_path" == */ ]] || sub_loc_path="${sub_loc_path}/"
        sub_loc="$(proxy_location "$sub_loc_path" "http://$(_upstream):${SUB_PORT}" "")"
        sub_loc="${sub_loc}"$'\n\n'
    fi
    ...
    if [[ -n "$PANEL_PATH" && "$PANEL_PATH" != "/" && "$PANEL_PATH" != "./" ]]; then
        panel_loc="$(proxy_location "$PANEL_PATH" "${PANEL_PROTO}://$(_upstream):${PANEL_PORT}" "$up_extra")"
        panel_loc="${panel_loc}"$'\n\n'
    fi
    ...
    if [[ -n "$panel_loc" ]]; then
        ...
        root_loc="$(proxy_location "/" "${PANEL_PROTO}://$(_upstream):${PANEL_PORT}" "$up_extra")"
    fi
    ...
    if [[ "$STREAM_443_MASTER" == "1" && -n "$root_dom" && "$root_dom" != "$PANEL_HOST" \
        && -f "/etc/letsencrypt/live/${root_dom}/fullchain.pem" ]]; then
        root_taken="$(sqlite3 "$XUI_DB" "SELECT port FROM inbounds WHERE protocol IN ('vless','vmess','trojan') AND port != ${STREAM_MASTER_PORT:-443} AND (listen = '127.0.0.1' OR listen = '' OR listen IS NULL) AND stream_settings LIKE '%\"network\": \"tcp\"%' AND (stream_settings LIKE '%\"serverNames\": [\"$(sql_escape "$root_dom")\"]%' OR stream_settings LIKE '%\"serverName\": \"$(sql_escape "$root_dom")\"%') LIMIT 1;" 2>/dev/null || true)"
...
EOF
    if is_docker_mode; then
        # В Docker: nginx в контейнере, проверяем через docker exec
        if docker compose -f "$COMPOSE_FILE" exec -T nginx nginx -t >/dev/null 2>&1; then
            ...
            warn "nginx -t не прошёл в контейнере — откат конфигурации."
    ...
    rm -f "$bak"
    if ! is_docker_mode && command -v systemctl >/dev/null 2>&1 && systemctl is-active nginx >/dev/null 2>&1; then
        systemctl reload nginx >/dev/null 2>&1 || true
    fi
...
create_channel() {
    ...
    if [[ "$SECURITY" == "reality" ]]; then
        ...
        if [[ "$STREAM_443_MASTER" == "1" ]]; then
            REALITY_TARGET="$(_upstream):${REALITY_TARGET_PORT:-9443}"
        else
    ...
    if [[ "$TRANSPORT" == "xhttp" && "$STREAM_443_MASTER" == "1" ]]; then
        # Вариант «за nginx»: инбаунд сидит на внутреннем адресе и наружу не
        # смотрит — снаружи виден только 443, маршрутизацию по SNI делает
        # stream-мастер (ssl_preread + proxy_protocol).
        # bare-metal: 127.0.0.1 (только localhost); Docker: пусто (0.0.0.0 — доступен из другого контейнера).
        if is_docker_mode; then
            LISTEN=""
        else
            LISTEN="127.0.0.1"
...
FROM inbounds i
WHERE (i.listen = '127.0.0.1' OR i.listen = '' OR i.listen IS NULL)
  AND json_extract(i.stream_settings, '$.network') = 'xhttp'
